...
void funcionHorasGen() {
    ...
    delay(1000); 
    server.sendHeader("Access-Control-Allow-Origin", "*");
    server.send(200, "text/plain", "Hora Luz Cambiada"); }
...
void funcionLuzEnt() {
  ...
  server.sendHeader("Access-Control-Allow-Origin", "*");
  server.send(200, "text/plain", "cambiando LuzEnt");}
...
void setup() {
  ...
  pinMode(pinLedCas, OUTPUT);
  digitalWrite(pinLedCas, 0);

  pinMode(pinLedFig, OUTPUT);
  digitalWrite(pinLedFig, 0);

  pinMode(pinLedEnt, OUTPUT);
  digitalWrite(pinLedEnt, 0);
